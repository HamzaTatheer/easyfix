package com.easyfix.Application.bl.services;

import com.easyfix.Application.models.CustomerModel;
import com.easyfix.Application.models.WorkerModel;

import java.util.ArrayList;
public interface CustomerService{
    public int login(String email,String password) throws Exception; //done
    public CustomerModel getCustomerDetails(int cid) throws Exception;
    public int register(String name,String email,String password,String credit,String city,String area) throws Exception; //done
    public ArrayList<WorkerModel> getFavourites(int cid);
    public boolean addToFavourite(int cid,int wid) throws Exception;
    public boolean changePaymentMethod(int cid,String newMethod);//done
    public ArrayList<WorkerModel> getWorkersCloseBy(int cid);
    public boolean changeCity(int cid,String newCity);//done
    public boolean changeArea(int cid,String newArea);//done
    public boolean giveRating(int cid, int wid,int rating) throws Exception;//done
}